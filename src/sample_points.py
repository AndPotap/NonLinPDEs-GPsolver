...
def sampled_pts_rdm(N_domain, N_boundary, domain, time_dependent=False):
    ...
    if time_dependent == False:
        ...
        # interior nodes
        X_domain = onp.concatenate(
            (random.uniform(x1l, x1r, (N_domain, 1)), random.uniform(x2l, x2r, (N_domain, 1))), axis=1
        )
...
def sampled_pts_grid(N_domain, N_boundary, domain, time_dependent=False):
    ...
    if time_dependent == False:
        ...
        XXv_bd = onp.concatenate(
            (XX[0, 0 : N_pts + 1], XX[N_pts + 1, 0 : N_pts + 1], XX[0 : N_pts + 1, 0], XX[0 : N_pts + 1, N_pts + 1]),
            axis=0,
        )
        YYv_bd = onp.concatenate(
            (YY[0, 0 : N_pts + 1], YY[N_pts + 1, 0 : N_pts + 1], YY[0 : N_pts + 1, 0], YY[0 : N_pts + 1, N_pts + 1]),
            axis=0,
        )
