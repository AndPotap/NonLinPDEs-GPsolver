...
class Darcy_flow2d(object):
    ...
    def Gram_matrix(self, kernel="Gaussian", kernel_parameter=0.2, nugget=1e-10, nugget_type="adaptive"):
        Theta_u, Theta_a = Gram_matrix_assembly(
            self.X_domain, self.X_boundary, eqn="Darcy_flow2d", kernel=kernel, kernel_parameter=kernel_parameter
        )
        ...
        if nugget_type == "adaptive":
            ...
            temp = onp.concatenate(
                (
                    ratio_u[0] * onp.ones((1, self.N_domain)),
                    ratio_u[1] * onp.ones((1, self.N_domain)),
                    ratio_u[2] * onp.ones((1, self.N_domain)),
                    onp.ones((1, self.N_domain + self.N_boundary)),
                ),
                axis=1,
            )
            ...
            temp = onp.concatenate(
                (
                    ratio_a[0] * onp.ones((1, self.N_domain)),
                    ratio_a[1] * onp.ones((1, self.N_domain)),
                    onp.ones((1, self.N_domain)),
                ),
                axis=1,
            )
    ...
    def loss(self, z):
        ...
        return (
            jnp.dot(temp_a, temp_a)
            + jnp.dot(temp_u, temp_u)
            + (1 / self.noise_level**2) * jnp.sum((v0[: self.N_data] - self.data_u) ** 2)
        )
    ...
    def GN_loss(self, z, z_old):
        ...
        v2 = z[5 * self.N_domain : 6 * self.N_domain]
        v3 = (
            (-self.rhs_f) * (-jnp.exp(-w0_old)) * w0 + (-v1_old) * w1 + (-v2_old) * w2 + (-w1_old) * v1 + (-w2_old) * v2
        )
    ...
    def extend_sol(self, X_test):
        Theta_u_test, Theta_a_test = construct_Theta_test(
            X_test,
            self.X_domain,
            self.X_boundary,
            eqn="Darcy_flow2d",
