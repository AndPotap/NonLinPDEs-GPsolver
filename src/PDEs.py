...
class Nonlinear_elliptic2d(object):
    ...
    def Gram_matrix(self, kernel="Gaussian", kernel_parameter=0.2, nugget=1e-8, nugget_type="adaptive"):
        Theta = Gram_matrix_assembly(
            self.X_domain, self.X_boundary, eqn="Nonlinear_elliptic", kernel=kernel, kernel_parameter=kernel_parameter
        )
        ...
        if nugget_type == "adaptive":
            ...
            temp = jnp.concatenate(
                (ratio * jnp.ones((1, self.N_domain)), jnp.ones((1, self.N_domain + self.N_boundary))), axis=1
            )
    ...
    def GN_relaxed_method(self, max_iter=3, step_size=1, initial_sol="rdm", pen_lambda=1e-10, print_hist=True):
        ...
        for iter_step in range(1, max_iter + 1):
            temp = jnp.linalg.solve(
                self.Hessian_GN_relaxed(sol, sol, pen_lambda), self.grad_loss_relaxed(sol, pen_lambda)
            )
            sol = sol - step_size * temp
            loss_now = self.loss_relaxed(sol, pen_lambda)
            if jnp.isnan(loss_now):
...
class Burgers(object):
    ...
    def Gram_matrix(
        self, kernel="anisotropic_Gaussian", kernel_parameter=[1 / 3, 1 / 20], nugget=1e-5, nugget_type="adaptive"
    ):
        Theta = Gram_matrix_assembly(
            self.X_domain, self.X_boundary, eqn="Burgers", kernel=kernel, kernel_parameter=kernel_parameter
        )
        self.nugget_type = nugget_type
        self.nugget = nugget
        self.kernel = kernel
    ...
    def Hessian_GN(self, z):
        ...
        mtx1 = jnp.concatenate(
            (-self.alpha * jnp.diag(v2), -self.alpha * jnp.diag(v0), self.nu * jnp.eye(self.N_domain)), axis=1
        )
        ...
        mtx = mtx.at[2 * self.N_domain : 3 * self.N_domain, 2 * self.N_domain : 3 * self.N_domain].set(
...
class Eikonal(object):
    ...
    def Gram_matrix(self, kernel="Gaussian", kernel_parameter=0.2, nugget=1e-8, nugget_type="adaptive"):
        Theta = Gram_matrix_assembly(
            self.X_domain, self.X_boundary, eqn="Eikonal", kernel=kernel, kernel_parameter=kernel_parameter
        )
        self.nugget_type = nugget_type
        self.nugget = nugget
        self.kernel = kernel
